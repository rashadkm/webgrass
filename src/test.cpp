...
bool Startup::checkMapset(std::string directory, std::string cont) 
{ 	
	cout<<"fff"<<endl;
...
{
	bool isValidLocation = checkMapset(*it, m_GrassDataDirectory);
...
// bool isValidLocation = checkMapset(dir[0], m_GrassDataDirectory);
// if (isValidLocation) 
// 	{ cb->addItem(dir[0]); } 
// cb->addItem(dir[1]);
// cb->addItem(dir[2]);
...
{
	bool isValidLocation = checkMapset(*it2, m_GrassMapsetDirectory);
	cout<<isValidLocation<<endl;
	if (isValidLocation) 
		{ cb1->addItem(*it2); } 
}
...
void Startup::comboChanged(WString loc_string){
	...
	string hh="/home/mayank/webgrassdata";
	string gg=hh+(string)"/";
	...
	for(vector<string>::iterator it2 = directories.begin();it2!=directories.end();++it2)
{	
	bool isValidLocation = checkMapset(*it2, jj);
